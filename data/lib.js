...
window.SITE = (function () {
  ...
  const PAYOFF = {
    ...
    formula(r) {
      // Купонный варрант: выплата не зависит от того, НАСКОЛЬКО актив выше страйка, —
    ...
    // Купонный варрант (бинарная выплата): S ≥ K → floor + pay, ниже → floor.
    // floor — что платится ниже страйка: 0 у варрантной формы (клиент платит
    // премию и при недоходе теряет её целиком), 100 у облигации с условным купоном
  ...
  const calc = {
    ...
    breakeven(r) {
      ...
      // Купонный варрант: результат меняется СКАЧКОМ ровно на страйке —
    ...
    move(r) {
      ...
      // Купонный варрант: кадр обязан показать обе полки — и нулевую слева от страйка, и
      ...
      // чтобы карточка открывалась на сценарии, где купон сработал (как у купонного варранта).
      if (r.type === "rcdigital") {
        const K = r.strike || 100;
  ...
  function payoffChart(r, o) {
    ...
    // «Ноль» выплаты для типа: у варранта и купонного варранта выплата считается от
    ...
    // У купонного варранта правую половину кадра занимает полка выплаты, и подпись базовой
  ...
  // облигационная форма купонного варранта. Один источник на все поверхности: выражение
