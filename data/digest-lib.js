...
window.DF = (function () {
  ...
  // couponPct здесь — купон за ВЕСЬ СРОК в % номинала (его считает
  // admin-digest-derive.js из couponPa и срока): оси времени в кадре нет, и
  // годовая ставка дала бы и неверную высоту полки, и неверный безубыток.
  // Линию «актив 1:1» тут НЕ рисуем, в отличие от защиты капитала: полка справа
  // и так говорит, что рост держателю не достаётся, а наклонная опора прошла бы
  // ровно через подписи — кадр 300×130 их не разводит.
  function revconvSvg(p, color) {
    const K = Number(p.strikePct) || 100, cpn = Number(p.couponPct) || 0;
    const pay = s => Math.min(100, s / K * 100) + cpn;
    // Безубыток: S/K·100 + купон = 100. Купон ≥ 100% покрывает любое падение —
    // точки нет, и рисовать её было бы выдумкой.
    const be = cpn >= 100 ? null : K * (100 - cpn) / 100;
    ...
    const s0 = Math.max(0, Math.min(K - 60, be != null ? be - 15 : K - 60)), s1 = K + 40;
    const ymin = pay(s0), ymax = pay(s1) + (pay(s1) - ymin) * 0.16 + 2;
    const PADL = 8, PADR = 8, PADT = 15, PADB = 22;
    const X = s => PADL + (s - s0) / (s1 - s0) * (CW - PADL - PADR);
    const Y = v => CH - PADB - (v - ymin) / (ymax - ymin) * (CH - PADT - PADB);
    ...
    let o = sLine(PADL, Y(100), CW - PADR, Y(100), GRID, 1, "2 4") +
            sTxt(CW - PADR, Y(100) + 14, "номинал 100%", "end");
    o += sPath("M" + X(s0).toFixed(1) + " " + Y(pay(s0)).toFixed(1) +
               " L" + X(K).toFixed(1) + " " + Y(pay(K)).toFixed(1) +
               " L" + X(s1).toFixed(1) + " " + Y(pay(s1)).toFixed(1), color, 2.5) +
         sDia(X(K), Y(pay(K)), color);
    ...
    if (Math.abs(K - 100) > 0.01) {
      o += sLine(X(K), Y(pay(K)) + 5, X(K), CH - PADB + 6, GHOST, 1, "3 3") +
           sTxt(X(K), CH - 6, "K " + gnum(K), "middle");
    }
    if (be != null && be >= s0) {
      o += '<circle cx="' + X(be).toFixed(1) + '" cy="' + Y(100).toFixed(1) + '" r="3.2" fill="' + color + '"/>';
      if (X(be) > PADL + 46) o += sTxt(X(be) - 6, Y(100) - 5, "б/у " + gnum(be), "end", EM);
    }
    return o + sTxt(CW - PADR, PADT - 4, "купон " + gnum(cpn) + "% за срок", "end");
  }
  ...
  function payoffSvg(p, color) {
    // Идея БЕЗ payoff — реальный случай: воркер срезает объект, если тип выплаты
    // не в его белом списке. Без этой строки p.type бросал TypeError, отрисовка
    // выпуска обрывалась на этой идее и страница переставала отзываться на клики.
    // В печатной версии такая защита была (p = p or {}), в экранной — нет.
    p = p || {};
    const W = 300, H = 130, PAD = 16;
    const x = t => PAD + t * (W - PAD * 2), y = t => PAD + t * (H - PAD * 2);
    let el = "";
  ...
  function audienceOf(idea) {
    ...
    // Реверс-конвертибл — отдельно от общего купонного текста: там падение считается
    // от СТАРТА, а здесь от страйка, и купон даёт подушку до безубытка. Общая фраза
    // «готовы к снижению номинала, если актив упадёт» была бы строже правды.
    if ((idea.payoff || {}).type === "revconv") {
      const pf = idea.payoff, K = pf.strikePct != null ? pf.strikePct : 100;
      const be = pf.couponPct != null && pf.couponPct < 100 ? K * (100 - pf.couponPct) / 100 : null;
      return "Подходит, если вы готовы держать «" + a + "» и хотите купон, который выплачивается " +
        "при любом сценарии: номинал возвращается полностью, пока актив не ниже " + numTxt(K) + "%" +
        (be != null ? ", а купон компенсирует падение вплоть до " + numTxt(be) + "% — ниже начинается убыток" : "") + ".";
    }
  ...
  function riskOf(idea) {
    ...
    if ((idea.payoff || {}).type === "revconv") {
      const K = numTxt(idea.payoff.strikePct != null ? idea.payoff.strikePct : 100);
      return "Защиты капитала нет: если на погашении базовый актив ниже " + K +
        "%, номинал уменьшается пропорционально падению от этого уровня, и купон покрывает убыток лишь частично. " +
        "Роста выше " + K + "% держатель не получает. Дополнительно — кредитный риск эмитента облигации.";
    }
    if (idea.family === "coupon") return prot
