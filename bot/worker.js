...
{"news":[{"flag":"🌍","title":"Нефть не верит миру","text":"Переговоры буксуют, поставки под риском — премия за геополитику остаётся в цене."},{"flag":"🇺🇸","title":"Ставки давят","text":"Доходности длинных бумаг у максимумов, рынок ждёт сигнала ФРС."},{"flag":"🇷🇺","title":"Рынок без покупателя","text":"Объёмы торгов низкие, индекс у сопротивления — импульса нет."}],"products":[{"id":"AAA-1","headline":"📈 Рычаг на ОФЗ: платите 12% — рост берёте целиком","hook":"Ставки у максимумов, значит развернуться им есть куда: премия 12% от номинала, индикативно, а участие в росте полное."},{"id":"BBB-2","headline":"💴 Рубль без поддержки — заходите в юань","hook":"Пока покупателя на рынке нет, валютная экспозиция работает страховкой: вход 15% номинала, индикативно."},{"id":"CCC-3","headline":"⛽️ Нефть в напряжении — ускоренный вход","hook":"Геополитика держит премию в цене: рост в диапазоне до 110% считается с коэффициентом 200%."}]}`;
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      ...
      raw = await callLLM(provider, system, messages, env,
        { temperature: 0.6, maxTokens: attempt ? 2600 : 1400, model: env.POST_MODEL, json: true });
    } catch (e) {
