...
{"news":[
  ...
  {"flag":"🌏","title":"<тезис>","text":"<1–2 предложения по Азии из статьи (Китай, Япония, Гонконг, юань, торговля с КНР); до 300 знаков>","long":["<абзац>","<абзац>"]},
...
- Флаги строго в порядке схемы: news[1] всегда 🌍, news[2] всегда 🇺🇸, news[3] всегда 🌏 (Азия), news[4] всегда 🇷🇺.
- products: ровно 5 РАЗНЫХ продуктов, id бери ТОЛЬКО из [скобок] в начале строк каталога. Подбирай под темы статьи: разворот/снижение ставок → облигации и варранты на ОФЗ; слабый рубль → валютные активы (CNY, USD); сырьё/акции → соответствующие базовые активы; Азия (Китай, Гонконг, Япония, торговля с КНР, юань) → продукты на CSI 300, Hang Seng, CNYRUB. Если блок Азии в статье есть, хотя бы ОДИН из пяти продуктов должен быть на азиатский актив — с зацепкой из этого блока.
- ТИПЫ ПЭЙОФФА НЕ ПОВТОРЯЮТСЯ. Тип написан в скобках сразу после [id] в каталоге: (варрант), (дисконтная облигация), (защита капитала), (автоколл), (реверс-конвертибл), (бустер), (биржевой выпуск). Из пяти позиций допустимо максимум ОДНО совпадение по типу — значит разных типов должно быть не меньше четырёх. Пять варрантов подряд — брак. Если под тему просится ещё один продукт того же типа, а совпадение уже израсходовано, возьми на тот же базовый актив продукт ДРУГОГО типа.
...
{"news":[{"flag":"🌍","title":"Нефть не верит миру","text":"Переговоры буксуют, поставки под риском — премия за геополитику остаётся в цене.","long":["Переговоры о перемирии идут третью неделю без результата, а поставки из региона остаются под риском: два терминала работают с перебоями.","Рынок закладывает эту неопределённость в цену — премия за геополитику держится, несмотря на слабый спрос."]},{"flag":"🇺🇸","title":"Ставки давят","text":"Доходности длинных бумаг у максимумов, рынок ждёт сигнала ФРС.","long":["Доходность десятилетних бумаг у верхней границы диапазона, аукционы проходят со слабым спросом.","До заседания ФРС рынок не готов брать длинную дюрацию — деньги остаются в коротком конце кривой."]},{"flag":"🌏","title":"Китай торгует вопреки","text":"Экспорт КНР растёт двузначными темпами, профицит торговли снова у рекорда — тарифы поставки не сдерживают.","long":["Экспорт Китая продолжает расти двузначными темпами, а положительное сальдо торгового баланса вновь у рекордных значений.","Тарифное давление поставки пока не сдерживает: спрос на китайские товары сохраняется, и юань получает поддержку со стороны торгового баланса."]},{"flag":"🇷🇺","title":"Рынок без покупателя","text":"Объёмы торгов низкие, индекс у сопротивления — импульса нет.","long":["Объём торгов третий день ниже среднего, индекс упирается в сопротивление и откатывает.","Покупателя нет ни в первом эшелоне, ни в бумагах роста: инвесторы ждут ясности по ставке."]}],"products":[{"id":"AAA-1","headline":"📈 Рычаг на ОФЗ: платите 12% — рост берёте целиком","hook":"Ставки у максимумов, значит развернуться им есть куда: премия 12% от номинала, индикативно, а участие в росте полное."},{"id":"BBB-2","headline":"💴 Китай торгует — юань в цене","hook":"Профицит КНР у рекорда поддерживает юань: вход 15% номинала, индикативно, рост курса берёте целиком."},{"id":"CCC-3","headline":"⛽️ Нефть в напряжении — ускоренный вход","hook":"Геополитика держит премию в цене: рост в диапазоне до 110% считается с коэффициентом 200%."},{"id":"DDD-4","headline":"🛡 Просадка не страшна: защита 100%","hook":"Рынок без покупателя — вход по номиналу, участие в росте 90%, тело возвращается целиком."},{"id":"EEE-5","headline":"🏦 Купон на корзину банков","hook":"Пока индекс у сопротивления, платит время: купон при барьере 65%, автоотзыв на 120%."}]}`;
...
// Четвёртый слот — Азия (08.09.2026): в обзоре аналитика появился отдельный блок «Азия»
// между США и Россией, и продукты на CSI 300 / Hang Seng / CNYRUB подбираются под него.
const MORNING_FLAGS = ["🌍", "🇺🇸", "🌏", "🇷🇺"];
...
const MORNING_RUBRICS = { "🌍": "Мир", "🇺🇸": "США", "🌏": "Азия", "🇷🇺": "Россия" };
...
const MORNING_SLOTS = { "🌍": "world", "🇺🇸": "us", "🌏": "asia", "🇷🇺": "ru" };
// Азиатский базовый актив в строке каталога: по нему линт проверяет, что под блок
// Азии взят хотя бы один продукт. Появится новый актив (Nikkei, Alibaba) — дописать сюда.
const MORNING_ASIA_RE = /CSI\s*300|Hang\s*Seng|CNY|юан|Nikkei|Китай|Гонконг|Япони/i;
...
function morningLint(p, catLines, article) {
  ...
  if (news.length !== MORNING_FLAGS.length) probs.push("в news должно быть ровно " + MORNING_FLAGS.length + " блока (сейчас " + news.length + ")");
  // сухая рубрика вместо тезиса — главный источник «скучных» заголовков
  const RUBRIC = /^(геополитика|сша|рынок сша|россия|российский рынок|рынок рф|рынок|новости|макро|трежерис|treasur\w*|сырьё|нефть|азия|рынок азии|рынки азии|китай|кнр|япония)\.?$/i;
  news.forEach((b, i) => {
    const n = "news[" + (i + 1) + "]";
    ...
    if (i < MORNING_FLAGS.length && b.flag !== MORNING_FLAGS[i]) probs.push(n + ": flag должен быть " + MORNING_FLAGS[i] + " (порядок: мир, США, Азия, РФ)");
  ...
  // Блок Азии в статье есть → среди пяти продуктов обязан быть азиатский актив: ради
  // этого слот и заводился. Актив узнаём по строке КАТАЛОГА, не по зацепке — модель
  // могла бы упомянуть Китай в hook у продукта на Сбербанк. Каталог без азиатских
  // активов (фикстура, пустая доска) правило не проверяет: требовать невозможное нельзя.
  const asiaIdx = MORNING_FLAGS.indexOf("🌏");
  const asiaBlock = asiaIdx >= 0 ? news[asiaIdx] : null;
  if (asiaBlock && !isMorningStub(asiaBlock) && ids.get) {
    const catHasAsia = [...ids.values()].some((l) => MORNING_ASIA_RE.test(l));
    const picked = prods.slice(0, 5).some((pr) => pr && MORNING_ASIA_RE.test(ids.get(pr.id) || ""));
    if (catHasAsia && !picked) {
      probs.push("в статье есть блок Азии, а среди продуктов нет ни одного на азиатский актив — возьми продукт на CSI 300, Hang Seng или CNYRUB с зацепкой из этого блока");
    }
  }
...
async function generateMorning(env, article) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      ...
      // Бюджет поднят под long: развёрнутые версии четырёх тем — это ещё ~950 токенов
      // (с Азией стало четыре блока, 08.09.2026: 2400 хватало на три). Со старыми 1400
      // ответ обрезался бы на середине, и ретрай жёг бы попытку.
      raw = await callLLM(provider, system, messages, env,
        { temperature: 0.6, maxTokens: attempt ? 4400 : 3000, model: env.POST_MODEL, json: true });
    } catch (e) {
  ...
  // Флаг ставим ПО СЛОТУ схемы (мир/США/Азия/РФ) — перепутанные флаги рендер чинит сам;
  // блоки-заглушки «тема не покрыта» отфильтровываем: их в посте быть не должно.
  const blocks = parsed.news.slice(0, MORNING_FLAGS.length)
...
async function sendMorningDraft(env, article, chatId) {
  ...
  if (env.POST_KV) {
    ...
    const items = res.blocks.slice(0, MORNING_FLAGS.length).map((b) => ({
...
function fiSpec(p) {
  ...
  if (p.type === "warrant") {
    ...
    s.push("вход 100% номинала");
    if (p.couponPa != null) s.push("купон " + fiNum(p.couponPa) + "% годовых, безусловный");
...
function shellDesc(item) {
  ...
  if (item.type === "autocall" || item.type === "revconv") {
